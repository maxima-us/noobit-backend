...
class StratBase():
    ...
    def __init__(self, name: str, description: str, exchange: str, pair: list, timeframe: int, volume: int):
        self.name = name
        self.description = description
        #! generating a new number for each new instance ==> not ok ==> look it up in db at init
        #! strat_id is auto-generated by orm and unique
    ...
    async def register_to_db(self):
        ...
        if not check:
            ...
            await Strategy.create(name=self.name)
        else:
    ...
    async def backtest(self):
        ...
        file_path = f"{noobit_user_path}/data/{self.exchange.lower()}_{self.pair}_historical_trade_data_fix_api.csv"
        ...
        try:
            ...
            logger.warning("CSV File not does exist, run historical trades aggregator")
        except Exception as e:
            log_exception(logger, e)
        ...
        try:
            backtrader_extension.run(self.df, os.path.basename(inspect.getmodule(self).__file__).split(".")[0])
        except Exception as e:
            log_exception(logger, e)