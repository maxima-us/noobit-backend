...
from noobit.models.data.websockets.unsubscription.parse.base import BaseUnsubParser


logger = get_logger(__name__)

class KrakenUnsubParser(BaseUnsubParser):
    ...
    async def public(self, symbol, timeframe, depth, feed):
        ...
        try:
            data = {"event": "unsubscribe", "pair": [symbol.replace("-", "/")], "subscription": {"name": map_to_exchange[feed]}}
            if feed == "ohlc":
    ...
    async def private(self, feed):
        ...
        try:
            ...
            data = {"event": "unsubscribe", "subscription": {"name": map_to_exchange[feed], "token": ws_token.value["token"]}}
            return data
